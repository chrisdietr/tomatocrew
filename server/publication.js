Meteor.publish('activeUsers', function() {
  return Meteor.users.find({},
    {fields: {username: 1, 'status.online': 1, 'profile.avatarUrl': 1, 'profile.numPomodoros': 1, 'status.lastLogin.date': 1}, sort: {'status.lastLogin.date': -1}, limit: 50}
  );
});
...
Meteor.publish('tasksListUsers', function() {
  ...
  return Meteor.users.find({_id: {$in: userIds}},
    {fields: {username: 1, userId: 1, 'profile.avatarUrl': 1}, sort: {'status.lastLogin.date': -1}}
  );
});
